...
namespace PPTooltip {
	...
	public class PoiPoiTooltip : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, ISelectHandler, IDeselectHandler {
		...
		/// ツールチップ動的生成時初期化
		/// </summary>
		public void InitTooltip(RectTransform setTooltipRect, string setTooltipInfo, TooltipAnimation.ANIMATION_TYPE type) {
			// マウスイベントを受けないように停止
			enabled = false;
		...
		private bool ValidateInspectorSettings() {
			...
			if (!toolTipRect.gameObject.TryGetComponent<PoiPoiTooltipObject>(out _)) {
				return false;
			}
		...
		private void CreateTooltip() {
			...
			if (tooltipInstance.gameObject.TryGetComponent(out PoiPoiTooltipObject ppto)) {
				ppto.SetInfo(toolTipInfo);
            }
