using System;
using System.IO;
using System.Runtime.CompilerServices;

namespace QOI
{
    internal struct QOIDiff
    {
        public QOIDiff(QOIPixel prev, QOIPixel next)
        {
            R = (byte)(next.R - prev.R + 2);
            G = (byte)(next.G - prev.G + 2);
            B = (byte)(next.B - prev.B + 2);
        }

        public byte R;
        public byte G;
        public byte B;

        public bool IsSmall() => R <= 3 && G <= 3 && B <= 3;
    }

    internal struct QOILumaDiff
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public QOILumaDiff(QOIPixel prev, QOIPixel next)
        {
            var r = next.R - prev.R;
            var g = next.G - prev.G;
            var b = next.B - prev.B;
            G = (byte)(g + 32);
            RG = (byte)(r - g + 8);
            BG = (byte)(b - g + 8);
        }

        public byte G;
        public byte RG;
        public byte BG;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool IsSmall()
        {
            return G <= 63 && RG <= 15 && BG <= 15;
        }
    }

    public class QOIEncoder
    {
        private readonly BinaryWriter _binWriter;
        private readonly byte[] _input;
        private readonly int _width;
        private readonly int _height;
        private readonly QOIChannels _channels;
        private readonly QOIColorSpace _colorSpace;
        private readonly QOIPixel[] _cache;
        private QOIPixel _prev;
        private byte _runLength;

        private const int CACHE_SIZE = 64;
        private const int MAX_RUN_LENGTH = 62;

        private QOIEncoder(BinaryWriter binWriter, byte[] input, int width, int height, QOIChannels channels, QOIColorSpace colorSpace)
        {
            _binWriter = binWriter;
            _input = input;
            _width = width;
            _height = height;
            _channels = channels;
            _colorSpace = colorSpace;
            _cache = new QOIPixel[CACHE_SIZE];
            _prev = new QOIPixel(0, 0, 0, 255);
            _runLength = 0;
        }

        /// <summary>
        ///     Encode raw color bytes into a QOI image.
        /// </summary>
        /// <param name="input">The raw color bytes.</param>
        /// <param name="width">The width of the image.</param>
        /// <param name="height">The height of the image.</param>
        /// <param name="channels">The number of channels of the image.</param>
        /// <param name="colorSpace">The color space of the image.</param>
        /// <returns>The bytes of the encoded QOI image.</returns>
        public static byte[] Encode(byte[] input, int width, int height, QOIChannels channels, QOIColorSpace colorSpace)
        {
            using var memStream = new MemoryStream();

            using (var binWriter = new BinaryWriter(memStream))
            {
                var encoder = new QOIEncoder(binWriter, input, width, height, channels, colorSpace);
                encoder.Encode();
            }

            return memStream.ToArray();
        }

        private void Encode()
        {
            WriteHeader();
            WriteChunks();
            WriteEndMarker();
        }

        private void WriteBigEndian(int value)
        {
            _binWriter.Write((byte)((value >> 24) & 0xFF));
            _binWriter.Write((byte)((value >> 16) & 0xFF));
            _binWriter.Write((byte)((value >> 08) & 0xFF));
            _binWriter.Write((byte)((value >> 00) & 0xFF));
        }

        private void WriteHeader()
        {
            _binWriter.Write("qoif".ToUTF8Bytes());
            WriteBigEndian(_width);
            WriteBigEndian(_height);
            _binWriter.Write((byte)_channels);
            _binWriter.Write((byte)_colorSpace);
        }

        private void WriteEndMarker()
        {
            _binWriter.Write((byte)0x0);
            _binWriter.Write((byte)0);
            _binWriter.Write((byte)0);
            _binWriter.Write((byte)0);
            _binWriter.Write((byte)0);
            _binWriter.Write((byte)0);
            _binWriter.Write((byte)0);
            _binWriter.Write((byte)1);
        }

        private void WriteChunks()
        {
            var pixelSize = _channels switch
            {
                QOIChannels.Rgb => 3,
                QOIChannels.Rgba => 4,
                _ => throw new ArgumentOutOfRangeException()
            };

            // flip image vertically
            for (var y = 0; y < _height; y++)
            {
                var yCoord = (_height - y - 1) * _width;
                for (var x = 0; x < _width; x++)
                {
                    var i = (yCoord + x) * pixelSize; // Index for pixel in flipped image
                    // Span<byte> span = _input.AsSpan(i, 4);
                    WriteChunk(new QOIPixel(_input[i], _input[i + 1], _input[i + 2], _channels == QOIChannels.Rgba ? _input[i + 3] : (byte)255));
                }
            }

            if (_runLength != 0)
                WriteRunChunk();
        }



        private void WriteChunk(QOIPixel next)
        {
            while (true)
            {
                var index = CalculateIndex(next);
                if (_prev.Equals(next) && _runLength < MAX_RUN_LENGTH)
                {
                    _runLength++;
                    _cache[index] = next;
                }
                else if (_runLength > 0)
                {
                    WriteRunChunk();
                    _runLength = 0;
                    continue;
                }
                else if (_cache[index].Equals(next))
                    WriteIndexChunk(index);

                else if (_prev.A == next.A)
                {
                    var diff = new QOIDiff(_prev, next);
                    var lumaDiff = new QOILumaDiff(_prev, next);
                    if (diff.IsSmall())
                        WriteDiffChunk(diff);
                    else if (lumaDiff.IsSmall())
                        WriteLumaChunk(lumaDiff);
                    else
                        WriteRgbChunk(next);

                    _cache[index] = next;
                }
                else
                {
                    WriteRgbaChunk(next);
                    _cache[index] = next;
                }

                _prev = next;
                break;
            }
        }

        private void WriteRgbChunk(QOIPixel pixel)
        {
            _binWriter.Write((byte)0b11111110);
            _binWriter.Write(pixel.R);
            _binWriter.Write(pixel.G);
            _binWriter.Write(pixel.B);
        }

        private void WriteRgbaChunk(QOIPixel pixel)
        {
            _binWriter.Write((byte)0b11111111);
            _binWriter.Write(pixel.R);
            _binWriter.Write(pixel.G);
            _binWriter.Write(pixel.B);
            _binWriter.Write(pixel.A);
        }

        private void WriteIndexChunk(int index) => _binWriter.Write((byte)index);

        private void WriteDiffChunk(QOIDiff diff)
        {
            byte chunk = 0b_01_00_00_00;
            chunk |= (byte)(diff.R << 4);
            chunk |= (byte)(diff.G << 2);
            chunk |= (byte)(diff.B << 0);
            _binWriter.Write(chunk);
        }

        private void WriteLumaChunk(QOILumaDiff lumaDiff)
        {
            byte byte1 = 0b_10_000000;
            byte1 |= lumaDiff.G;
            byte byte2 = 0;
            byte2 |= (byte)(lumaDiff.RG << 4);
            byte2 |= (byte)(lumaDiff.BG << 0);
            _binWriter.Write(byte1);
            _binWriter.Write(byte2);
        }

        private void WriteRunChunk()
        {
            byte chunk = 0b_11_000000;
            chunk |= (byte)(_runLength - 1);
            _binWriter.Write(chunk);
        }

        // private int CalculateIndex(Pixel pixel) => (pixel.R * 3 + pixel.G * 5 + pixel.B * 7 + pixel.A * 11) % CACHE_SIZE;
        private int CalculateIndex(QOIPixel pixel) => (pixel.R * 3 + pixel.G * 5 + pixel.B * 7 + pixel.A * 11) & (CACHE_SIZE - 1); // 63 in your case
    }
}